...
// マクロ定義
//========================================
#define NUMBER_SIZE_X 20.0f                                 // 数字の横の大きさ
#define NUMBER_SIZE D3DXVECTOR3(NUMBER_SIZE_X, 40.0f, 0.0f) // 数字の大きさ
...
// スコアのコンストラクタ
//========================================
CScore::CScore() :CScene(OBJTYPE::OBJTYPE_SCORE)
{
    m_nScore = 0;
}
...
void CScore::SetScore(const int nDisplayScore, D3DXVECTOR3 pos, D3DXCOLOR col)
{
    ...
    float fSpace = -NUMBER_SIZE_X;  // 数字の間の間隔(若干左へずらして、倒した敵の真ん中辺りに見せる)
    ...
    for (nIndex = 0; nIndex < MAX_SCORE; nIndex++)
    {
        ...
        if(bCheckStart == true)
        {
            ...
            for (int nCntNumber = 0; nCntNumber < MAX_NUMBER; nCntNumber++)
            {
                ...
                {
                    // 配列に入れる
                    m_apNumber[nCntNumber] = CNumber::Create(D3DXVECTOR3(pos.x + fSpace, pos.y, 0.0f), NUMBER_SIZE, col, true); // trueは、一定時間で消える数字

                    // 番号を設定                                                                                                  // 番号を指定
                    m_apNumber[nCntNumber]->SetNumber(nAnswer);

                    // 間隔を加算
                    fSpace += NUMBER_SIZE_X;
