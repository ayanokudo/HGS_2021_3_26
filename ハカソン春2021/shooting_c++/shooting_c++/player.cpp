        ...
        if (pScene != NULL)
        {
            ...
            if (objType == CScene::OBJTYPE_ENEMY)
            {
                ...
                for (int nCntCollision = 0; nCntCollision < pEnemy->GetCntCollision(); nCntCollision++)
                {
                    ...
                    // 当たり判定を計算
                    if (CircleCollision(pos, playerCollisionSize.x,
                        posCollision, sizeCollision.x) == true)
                    {
                        if (pEnemy->GetType() == CEnemy::TYPE_FLY)
                        {
                            // 当たった判定
                            Hit();
                        }
                        else if (pEnemy->GetType() == CEnemy::TYPE_SKELETON)
                        {
                            // サウンドを取得
            ...
            else if (objType == CScene::OBJTYPE_ITEM)
            {
                ...
                // 当たり判定を計算
                if (CircleCollision(pos, playerCollisionSize.x,
                    pScene2D->GetPosition(), pScene2D->GetSize().x) == true)
                {
                    // サウンドを取得
                    CSound *pSound = CManager::GetSound();
